...
-- This migration creates a sample admin user for testing the SOC dashboard

-- Insert admin user into auth.users table
-- Note: In production, this should be done through Supabase Auth API
-- For development/testing purposes, we'll create the user directly
...
-- We'll use a subquery to get the user ID
...
-- Create some sample data for the admin user to test with
...
-- Grant additional permissions for admin user
...
CREATE POLICY "Admins can manage all threat intelligence" 
ON public.threat_intelligence 
FOR ALL 
USING (public.is_admin(auth.uid()));

CREATE POLICY "Admins can manage all KPI metrics" 
...
CREATE POLICY "Admins can manage all AI interactions" 
